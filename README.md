...
├── .github/
│   └── workflows/
│       └── deploy.yml          # Pipeline de deploy automático no Pages
├── assets/
│   ├── audio/                  # Trilha sonora e efeitos de som (SFX)
│   ├── models/                 # Arquivos 3D (.glb / .gltf)
│   └── textures/               # Texturas PBR e mapas emissivos
├── css/
│   ├── main.css                # Estilização do HUD, menus e modais
│   └── animations.css          # Transições de interface
├── js/
│   ├── config/
│   │   └── Constants.js        # Definições globais de física, cores e grid
│   ├── core/
│   │   ├── Engine.js           # Render loop do Three.js e gerenciamento de câmera
│   │   ├── InputHandler.js     # Captura de teclado, touch e swipes
│   │   ├── GridSystem.js       # Regras lógicas da matriz e colisões
│   │   └── Block.js            # Lógica de estados e movimentação por pivô
│   ├── managers/
│   │   ├── AudioManager.js     # Controle de áudio e variações de pitch
│   │   ├── LevelManager.js     # Carregador das fases via JSON
│   │   └── StorageManager.js   # Manipulador de LocalStorage (Save State)
│   ├── ui/
│   │   ├── MenuUI.js           # Gerenciador do Menu e Loja de Skins
│   │   └── HUD.js              # Contador de movimentos e modal de vitória
│   └──
